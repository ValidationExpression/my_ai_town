...
func _test_slot_state_matrix(
	...
	var read_only_slot := healthy_slot.duplicate(true)
	read_only_slot["slotId"] = "read-only-%s" % identity
	read_only_slot["state"] = "read_only"
	read_only_slot["continueAvailable"] = false
	read_only_slot["modelEditAvailable"] = false
	read_only_slot["modelEditDisabledReason"] = (
...
func _test_load_page_action() -> void:
	...
	for layout_value: Variant in [
		...
		for left_index in compact_actions.size():
			...
			for text_role: String in ["Title", "Body", "Recovery", "Detail"]:
				var text_label := mobile_screen.find_child(
					"slot-a%s" % text_role,
					true,
					false,
				) as Label
				for action: Button in compact_actions:
					_expect(
						text_label != null
						and action != null
						and not _physical_rect(text_label, mobile_screen).intersects(
							_physical_rect(action, mobile_screen),
						),
						"%s 的 %s 不会被 %s 操作区覆盖" % [
							String(layout[1]),
							text_label.name if text_label != null else "missing",
