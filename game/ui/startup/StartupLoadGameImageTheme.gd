...
const REVISION := "ui.startup.load-game.image-atlas-runtime-v4-fixed-rebind-text"
const ATLAS_PATH := (
...
const MODEL_EDIT_ACTION_PATH := (
	"res://assets/ui/resident_admission/runtime/"
	+ "resident_rebind_left_extension.png"
)
const MODEL_EDIT_TEXT_PATH := (
	"res://assets/ui/resident_admission/runtime/"
	+ "resident_rebind_text_311x145.png"
)
...
static func _register_model_edit_button(
	theme: Theme,
	variation: StringName,
) -> void:
	var texture := _load_texture(MODEL_EDIT_ACTION_PATH)
	if texture == null:
		push_error("加载游戏页居民改绑按钮资产缺失。")
		return
	theme.set_type_variation(variation, &"Button")
	theme.set_font(
		&"font",
	...
	theme.set_font_size(&"font_size", variation, 18)
	for color_name: StringName in [
		&"font_color",
		&"font_hover_color",
		&"font_pressed_color",
	...
	theme.set_color(&"font_disabled_color", variation, Color("756652"))
	theme.set_constant(&"outline_size", variation, 0)
	theme.set_stylebox(
		&"normal",
		variation,
		_model_edit_style(texture, Color.WHITE, false),
	)
	theme.set_stylebox(
		&"hover",
		variation,
		_model_edit_style(texture, Color("fff0c9"), false),
	)
	theme.set_stylebox(
		&"pressed",
		variation,
		_model_edit_style(texture, Color("cbb58e"), true),
	)
	theme.set_stylebox(
		&"focus",
		variation,
	...
	theme.set_stylebox(
		&"disabled",
		variation,
		_model_edit_style(texture, Color("928a78"), false),
	)
...
static func _model_edit_style(
	...
	var style := StyleBoxTexture.new()
	style.texture = texture
	# The approved rebind asset reserves its right third for the resident/gear
	# icon. Keep the label inside the parchment extension on the left.
	style.content_margin_left = 7.0
	style.content_margin_top = 7.0 if not pressed else 9.0
	style.content_margin_right = 49.0
