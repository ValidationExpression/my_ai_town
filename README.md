...
- 减少启动和继续游戏时重复检查同一模型连接的次数；手动检查连接仍会即时重新验证，居民进入小镇时不会因此跳过必要的连接确认。
- 缩短居民记忆和化身记忆整理时使用的固定规则，减少后台整理的输入用量，同时保留事实边界、连续性和字段检查。
- 压缩居民普通决策中没有内容的动态区块；没有公告、活动、任务或对话时不再重复发送空白提示，同时保留可执行选项和关键事实边界。
- 修复多次恢复存档后世界日志时间线过长，导致 World 与居民状态都已保存却无法发布下一份完整存档的问题；原有完整修订会继续保留。
